...
describe('AbsoluteRenderOrderGroup', function () {
  ...
  it('check render order -- simple, no subgroups', function () {
    ...
    assert.equal(
      red,
      255,
      'Did not find red pixel, ordering is possibly incorrect.  Red amount found was: ' +
        red
    );
  });
  ...
  it('Test AbsoluteRenderOrderGroup correctly interleaves the ordering of subgroups', function () {
    ...
    assert.equal(
      red1,
      255,
      'Did not find correct amount of red in bright red pixel for test 1, ordering is possibly incorrect.  Red amount found was: ' +
        red1
    );
    assert.equal(
      red2,
      170,
      'Did not find correct amount of red in medium bright red pixel for test 1, ordering is possibly incorrect.  Red amount found was: ' +
        red2
    );
    assert.equal(
      red3,
      119,
      'Did not find correct amount of red in medium dark red pixel for test 1, ordering is possibly incorrect.  Red amount found was: ' +
        red3
    );
    assert.equal(
      black,
      0,
      'Did not find correct amount of red in black pixel for test 1, ordering is possibly incorrect.  Red amount found was: ' +
        black
    );
  });
});
