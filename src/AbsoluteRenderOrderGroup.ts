...
export class AbsoluteRenderOrderGroup extends Group {
  ...
  _drawChildren(drawMethod, canvas, top) {
    ...
    let orderedChildren = new Map<number, Node[]>(
  ...
  private addChildrenRecursivelyToMap(
    node: Node,
    orderedChildren: Map<number, Array<Node>>
  ): void {
    let rootNode: AbsoluteRenderOrderGroup = this;

    if (
      node == rootNode || // the AbsoluteRenderOrderGroup itself will always render using z-order logic, even if cached
      (node instanceof Group && !node.isCached())
    ) {
      // However, cached subgroups are considered to be just a regular object (this protects masking)
